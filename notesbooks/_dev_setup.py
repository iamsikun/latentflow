...
# --- Confirm import path ---
spec = importlib.util.find_spec("latentflow")
if spec and getattr(spec, "submodule_search_locations", None):
    pkg_dir = list(spec.submodule_search_locations)[0]
    print(f"✅ Using latentflow from: {pkg_dir}")
else:
    print(
        "❌ Could not import latentflow. Ensure src/latentflow exists or install with 'pip install -e .'"
    )
