...
from latentflow.models.hmm import GaussianARHMM
...
if __name__ == "__main__":
    ...
    arhmm_params = make_random_gaussian_arhmm(
        n_states=n_states,
    ...
    # Sample trajectory
    true_states, obs = sample_gaussian_arhmm(
        arhmm_params,
        T=T,
        rng=rng,
        history=history,
    ...
    # Create ARHMM model
    arhmm = GaussianARHMM(n_components=n_states, order=order, random_state=seed)
    ...
    _, axes[0] = plot_hmm_series_with_states(
        ...
        title="True Gaussian ARHMM Process",
    ...
    _, axes[1] = plot_hmm_series_with_states(
        ...
        pred_states,
        covariate_names=[f"cov{i+1}" for i in range(n_features)],
        title="Predicted Gaussian ARHMM Process",
        annotate_states=False,
        boundary_markers=True,
        ax=axes[1],
    )
